package mypackage.h2hub.fragments.auth
...
import mypackage.h2hub.databinding.FragmentLoginBinding
import mypackage.h2hub.fragments.auth.viewmodel.LoginViewModel
import mypackage.h2hub.utils.CheckInternet
                            ...
                            .addOnCompleteListener(requireActivity()) { task ->
                                ...
                                if (task.isSuccessful) {
                                    if (currentUser!!.isEmailVerified) {
                                        ...
                                        /*if (uid != null) {
                                            viewModel.getUserType(uid)
                                        }
                                        val result = viewModel.userType.data*/
                                        // Save user ID or any necessary data in SharedPreferences
                                        /*val sharedPref = requireActivity().getSharedPreferences("userType", Context.MODE_PRIVATE)
                                        val editor = sharedPref.edit()
                                        editor.putString("user_type", result.toString())
                                        editor.apply()*/
